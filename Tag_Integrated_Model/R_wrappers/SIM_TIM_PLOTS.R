...
# This version is one that will run sims, saves a 
# overall report of the runs (convergence rates/etc), build a .Rdat file with the important 
# results, and generate a simulation summary with plots/statistics. The user will need 
# To make choices which components of the code will be run. This allows code chunks to be 
# run or skipped.
# Good Luck and Enjoy!
#
############################################################################
...
##################################################################################
# VALUE GRAB
# This section of code grabs the values from all the report files and creates a 
# master .Rdat file for later use in plots/summaries etc
##################################################################################
...
if(keep.all==1){
  copy<-list.files(paste0(diag_direct,"\\Results_NOT_converged",sep=""))
...
#load results for generating plots
setwd(diag_direct)
...
R_apport_df_sim<-matrix(NA,(nyrs-1)*nreg_OM,nconv)
