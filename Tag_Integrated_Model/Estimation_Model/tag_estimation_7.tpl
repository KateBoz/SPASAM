...
//tagging data parameters
  ...
  init_4darray ntags(1,np,1,nreg,1,ny_rel,1,na) //releases
     !! int recap_index=np*nreg(1); // using this to dimension down arrays
   init_5darray OBS_recaps_temp(1,recap_index,1,ny_rel,1,na,1,tag_age,1,2)
...
// probably need to calculate quantity below  or omit *dh
   ...
   6darray OBS_recaps(1,np,1,nreg,1,ny_rel,1,na,1,tag_age,1,nreg) // for filling for calcs later

   init_4darray init_abund(1,np,1,np,1,nreg,1,na);  // need to calculate this or input it, we may want rec_devs before start of model? *dh
...
// need these for some if the init parameter arrays below
 ...
 //movement paramters
   init_3darray ln_T_est_pop(1,nyrs,1,nps,1,nps-1,phase_T_pop);  //movement parameters
 ...
 // selectivity parameters
  ...
  init_bounded_matrix log_sel_beta1surv(1,parpops,1,survfleet,-40,40,ph_sel_log);   //selectivity slope parameter 1 for logistic selectivity/double logistic
  init_bounded_matrix log_sel_beta2surv(1,parpops,1,survfleet,-50,20,ph_sel_log) ;  //selectivity inflection parameter 1 for logistic selectivity/double logistic
...
//recruitment parameters
  ...
  matrix rec_devs(1,nps,1,nyr) //derived quantity as exp(rec_devs_RN)
...
PROCEDURE_SECTION
 // initial calcs, can these types go to PRELIMINARY CALCS section?
  ...
  k=0;
    ...
    {
     k=k+1;
      OBS_recaps(i,r)=OBS_recaps_temp(k);
    }
    } 
  // assign catchabilities to arithmetic scale
       for(int i=1;i<=npops;i++) 
    {
      q_survey(i) = mfexp(ln_q); 
    }
          ...
          for (int z=1;z<=nfleets(j);z++)
           { 
             if(F_switch==1) //estimate annual deviations
              {
               F_year(j,r,y,z)=mfexp(ln_F(j,y,z)); //took out fleet here *dh*
              }
             if(F_switch==2) //random walk or AR1  in F if we ever want to try it.
              {
               if(y=1) //year one separate because no y-1
               {
               F_year(j,r,y,z)=mfexp(ln_F(j,r,y));  // took out fleet here *dh*
               }
               if(y>1)
               {
               //DG
               ...
               //DG
               F_year(j,r,y,z)=F_rho(j,r,z)*mfexp(ln_F(j,r,y-1));  //took out fleet here *dh*
               }
              }             
...

    // rec_devs=exp(ln_rec_devs_RN);  // gotta get this on the arithmetic scale yo...
          ...
          for(int j=1;j<=npops;j++) //recap stock
            {
             ...
             tag_like -= log_negbinomial_density(OBS_recaps(i,n,x,a,y,r),recaps(i,n,x,a,y,j,r)+0.00001,theta);  //negative binomial tag likelihood
            }}}}}}} 
 ...
 */
  ...
  */
  ...
  report<<"tag_like"<<endl<<tag_like<<"fish_age_like"<<endl<<fish_age_like;
  report<<"survey_like"<<endl<<survey_like;
  report<<"catch_like"<<endl<<catch_like;
  report<<"rec_like"<<endl<<rec_like;
RUNTIME_SECTION
