...
///////////////////////////////////////
 ...
 // !!exit(54);
    vector offset(1,3);                                    // Multinomial "offset"
 // !!exit(54);
LOCAL_CALCS
    ...
    for(int x=1; x<=nyrs_release; x++)
     {
      ...
      for (int a=1;a<=nages;a++) //release age //because accounting for release age, don't need to account for recap age, just adjust mortality and T, to use plus group value if recapture age exceeds max age
        ...
        if(ntags(i,n,x,a)==0)
         ...
         for(int s=1;s<=(max_life_tags*sum_nregions+1);s++) //create temp array that has columns of recap prob for each release cohort and add not recap probability to final entry of temp array
           {
           if(diagnostics_switch==1){
            ...
            if(max_life_tags<=(nyrs-xx+1)) //complete cohorts
             {
              offset(3) -= OBS_tag_prop_N(i,n,x,a) * ((tag_prop_final_TRUE(i,n,x,a,s)+0.001)*log(tag_prop_final_TRUE(i,n,x,a,s)+0.001));
             }
            if(max_life_tags>(nyrs-xx+1)) //need special calcs for incomplete cohorts (ie model ends before end max_life_tags reached)
             {
               if(s<((nyrs-xx+1)*sum_nregions+1))
                {
                 offset(3) -=OBS_tag_prop_N(i,n,x,a)*((tag_prop_final_TRUE(i,n,x,a,s)+0.001)*log(tag_prop_final_TRUE(i,n,x,a,s)+0.001));
                }
               if(s==((nyrs-xx+1)*sum_nregions+1))
                {
                 offset(3) -= OBS_tag_prop_N(i,n,x,a)*((tag_prop_final_TRUE(i,n,x,a,(max_life_tags*sum_nregions+1))+0.001)*log(tag_prop_final_TRUE(i,n,x,a,(max_life_tags*sum_nregions+1))+0.001));
                }
                }

            }}}}}

   sum_nregions=sum(nregions);
PARAMETER_SECTION
...
// need these for some if the init parameter arrays below
 ...
 ///#####################################################################################################
     init_bounded_matrix ln_T_YR(1,T_lgth_YR,1,T_lgth-1,-10,3,phase_T_YR);  
...
FUNCTION get_movement
  ...
  //combined a bunch of the loops here to just use one
  ...
  if(move_switch>1)
   {
    if(phase_T_YR>0) 
    {
      ...
      G=0;
      G_temp=0;
      for (int j=1;j<=sum_nregions;j++)
       {
        for (int i=1;i<=sum_nregions;i++) 
         {
            if(j==i)
            {
            G(j,i)=1;
            ...
            if(i>j)
            {
            G(j,i)=mfexp(ln_T_YR(j+sum_nregions*(y-1),i-1));
            }
            if(j!=i && i<j)
            {
            G(j,i)=mfexp(ln_T_YR(j+sum_nregions*(y-1),i));
            }
        }
       }    }
        G_temp=rowsum(G);     
          ...
          for (int k=1;k<=npops;k++)
           {
            ...
            if(npops==1 && sum_nregions==1) //if panmictic then movement is 100%
             {
             	T(j,r,y,a,k,n)=1;
            }
             if(move_switch>1)
   ...
   // didn't combine these two because they are extracting different things
  for (int j=1;j<=npops;j++)
   {
    for (int r=1;r<=nregions(j);r++)
     {
    ...
    G_reg=0;
     G_reg_temp=0;
      ...
      for (int j=1;j<=npops;j++)
       ...
       if(ph_reg_init<0 && sum_nregions==npops)
        {
         for (int j=1;j<=npops;j++)
          {
          for (int k=1;k<=npops;k++)
           ...
           for(int j=1;j<=npops;j++) //recap stock
            {
             for (int r=1;r<=nregions(j);r++)
              {
               tag_prop_temp2(i,n,x,a,((y-1)*sum_nregions+nreg_temp(j)+r))=tag_prop_temp(j,y,r); //stack array into single vector by year, population, region
              }
             }
            }
             for(int s=1;s<=(max_life_tags*sum_nregions+1);s++) //create temp array that has columns of recap prob for each release cohort and add not recap probability to final entry of temp array
              {
              if(s<(min(max_life_tags,nyrs-xx+1)*sum_nregions+1))
              {
               tag_prop_final(i,n,x,a,s)=tag_prop_temp2(i,n,x,a,s);
              }
              if(s==(min(max_life_tags,nyrs-xx+1)*sum_nregions+1) && max_life_tags<=(nyrs-xx+1)) //add not recap probability to final entry of temp array
              {
               tag_prop_final(i,n,x,a,s)=tag_prop_not_rec(i,n,x,a);  //for estimation model will use this version of tag_prop in likelihood
              }
              if(s==(min(max_life_tags,nyrs-xx+1)*sum_nregions+1) && max_life_tags>(nyrs-xx+1)) //add not recap probability to final entry of temp array with adjustment for release events where model ends before max_life_tags (so NR remains in last state)
              {
               tag_prop_final(i,n,x,a,(max_life_tags*sum_nregions+1))=tag_prop_not_rec(i,n,x,a);  //for estimation model will use this version of tag_prop in likelihood
              }
            }
           }
          }
  ...
  for (int y=1;y<=nyrs;y++) //need to alter to fit number of years of catch data
   {
     for (int j=1;j<=npops;j++)
      ...
      for (int i=1;i<=npops-1;i++) 
       {
          if(ln_T_YR(j+sum_nregions*(y-1),i)<Tpen)
           {
           Tpen_like+=(Tpen-ln_T_YR(j+sum_nregions*(y-1),i))*(Tpen-ln_T_YR(j+sum_nregions*(y-1),i));
          }
          if(ln_T_YR(j+sum_nregions*(y-1),i)>Tpen2)
           {
           Tpen_like+=(Tpen2-ln_T_YR(j+sum_nregions*(y-1),i))*(Tpen2-ln_T_YR(j+sum_nregions*(y-1),i));
          }
         }
        }
       }
            ...
            if(max_life_tags>(nyrs-xx+1)) //need special calcs for incomplete cohorts (ie model ends before end max_life_tags reached)
             {
               ...
               if(s==((nyrs-xx+1)*sum_nregions+1))
                {
                 tag_like_temp += ((OBS_tag_prop_final(i,n,x,a,(max_life_tags*sum_nregions+1))+0.001)*log(tag_prop_final(i,n,x,a,(max_life_tags*sum_nregions+1))+0.001));
                }
...
RUNTIME_SECTION
  ...
  maximum_function_evaluations 100 100 200 200 4000 
