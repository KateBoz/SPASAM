...
optimize.by.fleet<-'FALSE'             # optimize MSY by fleets within areas (=TRUE) or simply by using an F multiplier within an area (=FALSE)....latter reduces dimensionality of F permuations when number of fleets is too large to do full analysis
F.name<-"input_F"                                           # F values from .dat file to replace when iterating over F
...
F.seq<-seq(0,F.end,interval)                                                      # Create the desired F values to iterate over
...
MSY<-function(WD,ntrial,F.grab,perm)
 {
  ...
  invisible(file.copy(from=paste0(WD,"/Spatial_BRP.exe",sep=""),to=paste0(WD,"/MSY Results/Run",ntrial,"/Spatial_BRP.exe",sep="")))
  ...
  par_names=c('overlap_switch','larval_move_switch','move_switch','F','biomass_AM','biomass_stock','biomass_total','yield_fleet','yield_region','yield_stock','yield_total',
              'harvest_rate_region_bio','harvest_rate_stock_bio','harvest_rate_total_bio','depletion_region','depletion_stock','depletion_total',
              'SSB_region','SSB_stock','SSB_total','Bratio_stock','Bratio_total',
              'biomass_AM_overlap_region','biomass_stock_overlap','biomass_natal_overlap','yield_region_overlap','yield_stock_overlap','yield_natal_overlap',
              'harvest_rate_region_bio_overlap','harvest_rate_stock_bio_overlap','harvest_rate_natal_bio_overlap','depletion_region_overlap','depletion_stock_overlap',
  ...
  { 
    c(i,F.region1,bio_region[1],bio_region[nyrs],bio_stock[1],bio_stock[nyrs],bio_total[1],bio_total[nyrs],
      ...
      SSB_region[1],SSB_region[nyrs],SSB_stock[1],SSB_stock[nyrs],SSB_total[1],SSB_total[nyrs],Bratio_stock[1],Bratio_stock[nyrs],Bratio_total[1],Bratio_total[nyrs],
      ...
      yield_region_overlap[1],yield_region_overlap[nyrs],yield_stock_overlap[1],yield_stock_overlap[nyrs],yield_total_overlap[1],yield_total_overlap[nyrs],
      u_region_overlap[1],u_region_overlap[nyrs],u_stock_overlap[1],u_stock_overlap[nyrs],u_total_overlap[1],u_total_overlap[nyrs],
      dep_region_overlap[1],dep_region_overlap[nyrs],dep_stock_overlap[1],dep_stock_overlap[nyrs],dep_total_overlap[1],dep_total_overlap[nyrs],
      SSB_region_overlap[1],SSB_region_overlap[nyrs],SSB_stock_overlap[1],SSB_stock_overlap[nyrs],SSB_total_overlap[1],SSB_total_overlap[nyrs],Bratio_stock_overlap[1],Bratio_stock_overlap[nyrs],Bratio_natal_overlap[1],Bratio_natal_overlap[nyrs]
    )
  }else{
  if(sum(nstocks)<2  & sum(nregions)>1)
  {
 c(i,rbind(F.region1[,]),rbind(bio_region[,1]),rbind(bio_region[,nyrs]),rbind(bio_stock[1]),rbind(bio_stock[nyrs]),bio_total[1],bio_total[nyrs],
    rbind(yield_region[,1]),rbind(yield_region[,nyrs]),rbind(yield_stock[1]),rbind(yield_stock[nyrs]),yield_total[1],yield_total[nyrs],
    rbind(u_region[,1]),rbind(u_region[,nyrs]),rbind(u_stock[1]),rbind(u_stock[nyrs]),u_total[1],u_total[nyrs],
    rbind(dep_region[,1]),rbind(dep_region[,nyrs]),rbind(dep_stock[1]),rbind(dep_stock[nyrs]),dep_total[1],dep_total[nyrs],
    rbind(SSB_region[,1]),rbind(SSB_region[,nyrs]),rbind(SSB_stock[1]),rbind(SSB_stock[nyrs]),SSB_total[1],SSB_total[nyrs],Bratio_stock[1],Bratio_stock[nyrs],Bratio_total[1],Bratio_total[nyrs],
    rbind(bio_region_overlap[,1]),rbind(bio_region_overlap[,nyrs]),rbind(bio_stock_overlap[1]),rbind(bio_stock_overlap[nyrs]),bio_total_overlap[1],bio_total_overlap[nyrs],
    rbind(yield_region_overlap[,1]),rbind(yield_region_overlap[,nyrs]),rbind(yield_stock_overlap[1]),rbind(yield_stock_overlap[nyrs]),yield_total_overlap[1],yield_total_overlap[nyrs],
    rbind(u_region_overlap[,1]),rbind(u_region_overlap[,nyrs]),rbind(u_stock_overlap[1]),rbind(u_stock_overlap[nyrs]),u_total_overlap[1],u_total_overlap[nyrs],
    rbind(dep_region_overlap[,1]),rbind(dep_region_overlap[,nyrs]),rbind(dep_stock_overlap[1]),rbind(dep_stock_overlap[nyrs]),dep_total_overlap[1],dep_total_overlap[nyrs],
    rbind(SSB_region_overlap[,1]),rbind(SSB_region_overlap[,nyrs]),rbind(SSB_stock_overlap[1]),rbind(SSB_stock_overlap[nyrs]),SSB_total_overlap[1],SSB_total_overlap[nyrs],Bratio_stock_overlap[1],Bratio_stock_overlap[nyrs],Bratio_natal_overlap[1],Bratio_natal_overlap[nyrs]
  )
  }else{
  if(sum(nstocks)>1 & sum(nregions)>1)
    {
     matrix(c(i,as.vector(t(F.region1)),rbind(bio_region[,1]),rbind(bio_region[,nyrs]),rbind(bio_stock[,1]),rbind(bio_stock[,nyrs]),bio_total[1],bio_total[nyrs],
        rbind(yield_region[,1]),rbind(yield_region[,nyrs]),rbind(yield_stock[,1]),rbind(yield_stock[,nyrs]),yield_total[1],yield_total[nyrs],
        rbind(u_region[,1]),rbind(u_region[,nyrs]),rbind(u_stock[,1]),rbind(u_stock[,nyrs]),u_total[1],u_total[nyrs],
        rbind(dep_region[,1]),rbind(dep_region[,nyrs]),rbind(dep_stock[,1]),rbind(dep_stock[,nyrs]),dep_total[1],dep_total[nyrs],
        rbind(SSB_region[,1]),rbind(SSB_region[,nyrs]),rbind(SSB_stock[,1]),rbind(SSB_stock[,nyrs]),SSB_total[1],SSB_total[nyrs],Bratio_stock[,1],Bratio_stock[,nyrs],Bratio_total[1],Bratio_total[nyrs],
...
u<-graph$u_tot_end
MSY1<-graph$yield_tot_end
SPR<-graph$Bratio_tot_end
bio<-graph$bio_tot_end

stock<-graph[which.max(graph$yield_tot_end),]
